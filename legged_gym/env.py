...
from legged_gym import LEGGED_GYM_ROOT_DIR, LEGGED_GYM_ASSET_DIR
...
class Env:
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.env.headless == True:
            self.graphics_device_id = -1

        self.num_envs = cfg.env.num_envs
        self.num_obs = cfg.env.num_observations * cfg.env.state_hist
        self.unit_obs = cfg.env.unit_obs
        self.num_privileged_obs = cfg.env.num_privileged_obs * cfg.env.state_hist + cfg.env.height_obs
        ...
        # print(type(self.num_envs), type(self.num_obs))
        ...
        if self.cfg.env.headless == False:
    ...
    def post_physics_step(self):
        ...
        )  # False apparently (must use quat_rotate) according to
        # https://github.com/leggedrobotics/legged_gym/compare/master...curieuxjy:legged_gym:master
    ...
    def compute_observations(self):
        ...
        self.sample_unit_obs = self.obs_buf

        # add noise if needed
        if self.add_noise:
            # print("obs buf size : ", self.obs_buf.shape)
            # print("sample unit obs size : ", self.sample_unit_obs.shape)
            # print("noise scale vec  : ", self.noise_scale_vec.shape)
            self.obs_buf += (2 * torch.rand_like(self.sample_unit_obs) - 1) * self.noise_scale_vec
        ...
        # print("init priv obs buf size : ", self.privileged_obs_buf.shape)

        self.unit_privileged_obs_buf = self.obs_buf
        # self.privileged_obs_buf[:, :self.obs_buf.shape[1]] = self.obs_buf

        # print("priv obs buf size init: ", self.unit_privileged_obs_buf.shape)
        ...
        if self.cfg.terrain.measure_heights:
            ...
            self.unit_privileged_obs_buf = torch.cat((self.unit_privileged_obs_buf, heights), dim=-1)
            # print("priv obs buf size after heights: ", self.unit_privileged_obs_buf.shape)
        ...
        if self.cfg.env.priv_observe_contact_forces:
            ...
            self.unit_privileged_obs_buf = torch.cat(
                (
                    self.unit_privileged_obs_buf,
                    (self.contact_forces.view(self.num_envs, -1) - contact_forces_shift) * contact_forces_scale,
                ),
                dim=-1,
            )
            # print("priv obs buf size after contacts: ", self.unit_privileged_obs_buf.shape)

            # print(self.privileged_obs_buf.shape) #[4000,51]

        if self.cfg.env.priv_observe_base_lin_vel:
            self.unit_privileged_obs_buf = torch.cat(
                (self.base_lin_vel * self.obs_scales.lin_vel, self.unit_privileged_obs_buf), dim=-1
            )
            # print("priv obs buf size after lin vel: ", self.unit_privileged_obs_buf.shape)

        self.privileged_obs_buf = self.unit_privileged_obs_buf
    ...
    def create_sim(self):
        ...
        # print("max_gpu_contact_pairs", sim_params.physx.max_gpu_contact_pairs)
    ...
    def _call_train_eval(self, func, env_ids):
        ...
        env_ids_train = env_ids[env_ids < self.num_envs]
        env_ids_eval = env_ids[env_ids >= self.num_envs]
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_base_com:
            ...
            props[0].com += gymapi.Vec3(
                np.random.uniform(rng[0], rng[1]), np.random.uniform(rng[0], rng[1]), np.random.uniform(rng[0], rng[1])
            )
        return props
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        """
        # print("obs buf 0 : ",self.obs_buf[0])
        # noise_vec = torch.zeros_like(self.obs_buf[0])
        noise_vec = torch.zeros_like(self.sample_unit_obs[0])
        # print("noise vec : ", noise_vec.shape)
    ...
    def _init_buffers(self):
        ...
        if self.cfg.control.control_type == "actuator_net":
            actuator_path = f"{LEGGED_GYM_ASSET_DIR}/actuator_nets/unitree_go1.pt"

            actuator_network = torch.jit.load(actuator_path).to(self.device)

            def eval_actuator_network(
                joint_pos, joint_pos_last, joint_pos_last_last, joint_vel, joint_vel_last, joint_vel_last_last
            ):
                xs = torch.cat(
                    (
    ...
    def _get_env_origins(self):
        ...
        if self.cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            # print("raw max init level", max_init_level)
    ...
    def _parse_cfg(self, cfg):
        ...
        self.cfg.domain_rand.rand_interval = float(np.ceil(self.cfg.domain_rand.rand_interval_s / self.dt))
        self.cfg.domain_rand.gravity_rand_interval = float(
            np.ceil(self.cfg.domain_rand.gravity_rand_interval_s / self.dt)
        )
        self.cfg.domain_rand.gravity_rand_duration = float(
            np.ceil(self.cfg.domain_rand.gravity_rand_interval * self.cfg.domain_rand.gravity_impulse_duration)
        )
    ...
    def _reward_action_smoothness_1(self):
        # Penalize changes in actions
        diff = torch.square(
            self.joint_pos_target[:, : self.num_actuated_dof] - self.last_joint_pos_target[:, : self.num_actuated_dof]
        )
    ...
    def _reward_action_smoothness_2(self):
        # Penalize changes in actions
        diff = torch.square(
            self.joint_pos_target[:, : self.num_actuated_dof]
            - 2 * self.last_joint_pos_target[:, : self.num_actuated_dof]
            + self.last_last_joint_pos_target[:, : self.num_actuated_dof]
        )
        diff = diff * (self.last_actions[:, : self.num_dof] != 0)  # ignore first step
    ...
    #     target_height = 0.05 * phases + 0.02 # offset for foot radius 2cm
